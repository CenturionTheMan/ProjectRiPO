...
\input{TexBase/DocumentBase.tex}
    ...
    % (??????) [Architektura logiczna systemu]  :> Dziedziak %?DONE


    \section{Wstęp}
    \input{Sections/Wstep.tex} % DONE??
        
    \section{Case studies}
    \input{Sections/CaseStudies.tex} % PRZYKŁADY Z DZIAŁANIA (FOTY)

    \section{Opis funkcjonalny}
    \input{Sections/OpisFuncjonalny.tex} % Wymagania funkcjonalne

    \section{Architektura wysokopoziomowa systemu}
    \input{Sections/ArchitekturaWysokopoziomowa.tex} % żerżnąć z jego przykładu i uszczegółowić

    \section{Architektura logiczna systemu}
    \input{Sections/ArchitekturaLogiczna.tex} %  ISO 42010 / https://www.youtube.com/watch?v=AGWdh4GO8Lw

    \section{Dobór technologii}
    \input{Sections/DoborTechnologii.tex}   % Coś nazmyślać

    \section{Planowany zakres prac rozwojowych}
    \input{Sections/PlanowanyZakresPrac.tex} % XD -> jakieś rysuneczki albo lista i fajrant

    \section{Plan testów systemu}
    \input{Sections/PlanTestowSystemu.tex} % 0????
